...
/* 按模式求误差: 角度卷绕 / 连续位置不卷绕 */
static float axis_error(const MotorPID *p, float e) {
    return p->angular ? wrap180(e) : e;
}
...
void MotorPID_Init(MotorPID *p) {
    ...
    p->vel_clamp      = 150.0f;    /* deg/s (默认, 可被上层覆盖) */

    p->angular    = 1;             /* 默认角度模式 (兼容旧行为) */
    p->enabled    = 0;
...
void MotorPID_SetMode(MotorPID *p, uint8_t angular) {
    p->angular = angular ? 1 : 0;
}
...
float MotorPID_GetError(const MotorPID *p) {
    return axis_error(p, p->target - p->current);
}
...
void MotorPID_Update(MotorPID *p, float angle_deg, float dt) {
    ...
    err = axis_error(p, p->target - angle_deg);
    ...
    p->filtered_vel = clampf(p->filtered_vel, -p->vel_clamp, p->vel_clamp);   /* deg/s */
