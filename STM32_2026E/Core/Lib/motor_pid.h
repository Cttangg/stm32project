...
typedef struct {
    ...
    float vel_clamp;      /* 测量速度限幅 deg/s (防传感器跳变; 须覆盖实际最高速) */

    uint8_t angular;      /* 误差模式: 1=角度卷绕±180°; 0=连续位置(多圈/直线)不卷绕 */
...
/**
 * @brief 设置误差模式 (须在 SetTarget 之前调用)
 * @param angular 1=角度(误差卷绕±180°); 0=连续位置(多圈/直线, 不卷绕)
 */
