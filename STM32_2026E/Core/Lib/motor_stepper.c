...
void MotorStepper_Enable(MotorStepper *s) {
    TMC2209_Enable(s->motor);
}

void MotorStepper_Disable(MotorStepper *s) {
    TMC2209_Disable(s->motor);
}

void MotorStepper_SetDirection(MotorStepper *s, TMC2209_Dir dir) {
    if (s->direction == (uint8_t)dir) return;
    /* 停表(ISR 不再触发) → 设 DIR; 下次启动首周期提供建立时间 */
...
void MotorStepper_SetMicrostep(MotorStepper *s, TMC2209_Microstep ms) {
    TMC2209_SetMicrostep(s->motor, ms);
}
...
void MotorStepper_SetVelocity(MotorStepper *s, float steps_per_sec) {
    ...
    if (s->direction != (uint8_t)dir) {
        ...
        s->direction = (uint8_t)dir;
        TMC2209_SetDirection(s->motor, dir);
    } else if (s->step_rate > 0.0f) {
        /* 同向且速度几乎不变: 不重配定时器 (闭环每周期调用时的防抖) */
        float d = rate - s->step_rate;
        if (d < 0.0f) d = -d;
        if (d < 0.5f) return;
    }
...
uint8_t MotorStepper_IsBusy(const MotorStepper *s) {
    uint32_t target = s->target_steps;   /* volatile 快照 */
    if (target == 0) return 0;
    return (s->step_count < target) ? 1 : 0;
}

uint32_t MotorStepper_GetRemainingSteps(const MotorStepper *s) {
    uint32_t target = s->target_steps;
    if (target == 0 || s->step_count >= target) return 0;
    return target - s->step_count;
}
