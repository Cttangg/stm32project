...
/** @brief 使能驱动 (EN 低有效), 转发到绑定的 TMC2209 */
void MotorStepper_Enable(MotorStepper *s);

/** @brief 禁止驱动 (EN 高), 转发到绑定的 TMC2209 */
void MotorStepper_Disable(MotorStepper *s);

/**
 * @brief 设置方向 (停表→设 DIR, 不启动; 供 MoveSteps 前调用)
 *        方向改变后由下一次启动的首个周期提供 DIR 建立时间
 */
void MotorStepper_SetDirection(MotorStepper *s, TMC2209_Dir dir);

/** @brief 设置细分 (转发到绑定的 TMC2209; 建议停机时调用) */
